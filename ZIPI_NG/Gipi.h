...
[ ZIPI_RunPista _h   count cur key done;
  ...
  do {
    ...
    switch (key) {
      'p', 'P':
        cur++;
        print "(", cur, "/", count, ") ", (string) _h.&ZIPI_pistas-->(cur - 1), "^^";
        break;
      'q', 'Q', 27, 131, 10, 8:
        return;
    }
...
[ ZIPI_RunMenuT _m cur count i j target;
  ...
  for (::) {
    ...
    if (j==-1000) {
      ...
      print "El número va de 1 a ", count, " (0=salir)";
      ZIPI_Espera();
    } else {
      if (j == 0) return;
...
[ ZIPI_RunMenuT _m cur count i j target;
  ...
  for (::) {
    ...
    print "^Pulsa con el ratón sobre una de las anteriores ";
    glk(256, 100);
    print "(o aquí para salir)";
    glk(256, 0);
    print "^^";
...
[ ZIPI_RunPistaT _h   count cur key done;
  ...
  print "^";
  glk(256, 1); print "Pulsa aquí para una pista"; glk(256, 0);
    ...
    switch (key) {
      1:
        ...
        if (count - cur > 1) {
          ...
          print " ¿O prefieres ";
          glk(256, 2); print "pensártelo?"; glk(256, 0);
        }
        break;
      2:
       return;
